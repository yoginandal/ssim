...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        console.log(`Multer destination: eventId from req.body.id = '${eventId}' for file ${file.originalname}`);
        if (!eventId) {
            console.error('Multer destination: Event ID is missing for file ' + file.originalname);
            return cb(new Error('Event ID is missing. Cannot determine upload folder.'));
        }
        const eventSpecificPath = path.join(uploadsDir, String(eventId));
        console.log(`Multer destination: Determined eventSpecificPath = '${eventSpecificPath}' for file ${file.originalname}`);
        try {
            if (!fs.existsSync(eventSpecificPath)) {
                fs.mkdirSync(eventSpecificPath, { recursive: true });
                console.log(`Multer destination: Directory CREATED: ${eventSpecificPath}`);
            } else {
                console.log(`Multer destination: Directory ALREADY EXISTS: ${eventSpecificPath}`);
            }
            cb(null, eventSpecificPath);
        } catch (mkdirError) {
            console.error(`Multer destination: FAILED to create/access directory ${eventSpecificPath}`, mkdirError);
            cb(mkdirError);
        }
    },
    filename: function (req, file, cb) {
        ...
        console.log(`Multer filename: originalname='${originalName}', ext='${ext}'`);
        // Add a random number component to ensure higher uniqueness for temporary filenames
        const randomComponent = Math.floor(Math.random() * 10000);
        const tempFilename = `${file.fieldname}-${Date.now()}-${randomComponent}${ext}`;
        console.log(`Multer filename: tempFilename='${tempFilename}' for original name '${originalName}'`);
...
const imageFileFilter = (req, file, cb) => {
    console.log(`File filter processing: originalname: ${file.originalname}, mimetype: ${file.mimetype}`); // Logging
    if (file.mimetype.startsWith('image/')) {
        cb(null, true);
    } else {
        console.log(`File rejected by filter: ${file.originalname}, mimetype: ${file.mimetype}`); // Logging
...
app.post('/api/events', upload.array('eventImages'), async (req, res) => {
    console.log('Route handler /api/events: req.files received from Multer:', JSON.stringify(req.files, null, 2)); // Logging
    ...
    if (!id || !title || !description) {
        return res.status(400).json({ message: 'Missing required event fields: id, title, description.' });
    }
    ...
    try {
        ...
        if (tempUploadedFiles && tempUploadedFiles.length > 0) {
            ...
            try {
                ...
                console.error(`Error reading directory ${eventSpecificPath} for sequencing:`, e);
                throw new Error(`Server error: Could not read event directory for image sequencing.`);
            }
            ...
            for (const tempFile of tempUploadedFiles) {
                ...
                console.log(`Route handler loop: Processing originalname: ${tempFile.originalname}, Extracted extension: ${extension}, Multer tempFile.path: ${tempFile.path}, Multer tempFile.destination: ${tempFile.destination}, Multer tempFile.filename: ${tempFile.filename}`);
                ...
                if (!fs.existsSync(tempFilePath)) {
                    console.error(`CRITICAL: Source file for rename does not exist or is inaccessible. Path: ${tempFilePath}`);
                    console.error(`tempFile details: destination: ${tempFile.destination}, filename: ${tempFile.filename}, originalname: ${tempFile.originalname}`);
                    throw new Error(`Failed to process uploaded image ${tempFile.originalname}. Reason: Temporary file not found at ${tempFilePath}.`);
                }
                ...
                try {
                    ...
                    console.error(`Error renaming file ${tempFilePath} to ${newFilePath}:`, renameError);
                    throw new Error(`Failed to process uploaded image ${tempFile.originalname}. Details: ${renameError.message || renameError.code || String(renameError)}`);
                }
            }
        }
        ...
        const eventQuery = 'INSERT INTO events (id, title, description) VALUES (?, ?, ?)';
        ...
        if (finalImagePathsForDB.length > 0) {
            const imageInsertQuery = 'INSERT INTO event_images (event_id, image_path) VALUES (?, ?)';
        ...
        console.error('Error saving event to database or processing images:', error);

        // Cleanup:
        // 1. Attempt to delete files that were successfully renamed in this batch.
        successfullyRenamedFiles.forEach(filePath => {
            fs.unlink(filePath, err => { 
                if (err) console.error('Error deleting successfully renamed file during rollback:', filePath, err);
            });
        });
        ...
        if (tempUploadedFiles && tempUploadedFiles.length > 0) {
            tempUploadedFiles.forEach(file => {
                fs.access(file.path, fs.constants.F_OK, (errAccess) => {
                    if (!errAccess) { // File exists at original temp path
                        fs.unlink(file.path, errUnlink => {
                            if (errUnlink) console.error('Error deleting temporary uploaded file after error:', file.path, errUnlink);
                        });
                    }
                });
            });
        ...
        if (error.code === 'ER_DUP_ENTRY' && error.message.includes('events.PRIMARY')) { 
             return res.status(409).json({ message: 'Conflict: Event with this ID already exists.', error: error.message });
        }
        const clientMessage = error.message && (error.message.startsWith('Failed to process') || error.message.startsWith('Server error:')) ?
                              error.message : 'Failed to save event.';
        return res.status(500).json({ message: clientMessage, errorDetails: error.toString() });
    } finally {
        if (connection) connection.release();
    }
});
...
app.get('/api/events', async (req, res) => {
    ...
    try {
        ...
        // 1. Fetch all events
        const [events] = await connection.query('SELECT id, title, description, created_at FROM events ORDER BY created_at DESC');
        ...
        for (const event of events) {
            const [images] = await connection.query('SELECT image_path FROM event_images WHERE event_id = ? ORDER BY uploaded_at ASC', [event.id]);
